...
gSheets.loadAuth = function(callback = null, params) {
  // Load client secrets from a local file.
  fs.readFile('./lib/sheets/credentials.json', (err, content) => {
    if (err) return console.log('Error loading client secret file:', err);
    // Authorize a client with credentials, then call the Google Sheets API.
    gSheets.authorize(JSON.parse(content), callback, params);
  });
};

gSheets.authorize = function(credentials, callback, params) {
...
gSheets.onAuthorized = function(auth, values) {
...
gSheets.updateSheet = function(spreadsheetId, sheetId, auth, vals) {
  let values = [
    vals
  ];
  const sheets = google.sheets({version: 'v4', auth});
